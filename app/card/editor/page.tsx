...
export default function CardEditor() {
  ...
  const cardBgStyle: React.CSSProperties | undefined = useMemo(() => {
    ...
    if (bgMode === 'gradient')
      return { background: gradientPresets[bgGradientKey] || gradientPresets.blue }
    return undefined
  ...
  const [ratioPreset, setRatioPreset] = useState<'xhs' | 'square' | 'story' | 'land' | 'custom'>(
    'xhs'
  )
  ...
  const [fontFamily, setFontFamily] = useState<string>(
    'system-ui, -apple-system, "SF Pro Text", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Noto Sans SC", Segoe UI, Roboto, Arial'
  )
  ...
  useEffect(() => {
    ...
    const waitReady = async (root: HTMLElement) => {
      ...
      try {
        if ((document as any).fonts?.ready) waits.push((document as any).fonts.ready)
      } catch {}
    ...
    const buildPaginate = async () => {
      ...
      if (debouncedMdx) {
        try {
          ...
          if (!gfmMod?.default && html) {
            ...
            // @ts-ignore
            root.render(
              <div className="mdx-content" style={{ fontSize: `${16 * fontScale}px` }}>
                <MDX />
              </div>
            )
            await new Promise((r) =>
      ...
      const blocks: Element[] =
        direct.length > 1
          ? direct
          : Array.from((direct[0] as HTMLElement | undefined)?.children || meas.children)
      ...
      const splitParagraph = (
        ...
        remaining: number
      ): { first?: HTMLElement; rest?: HTMLElement } => {
      ...
      for (const node of blocks) {
        ...
        const isBreak =
          ...
          (node as HTMLElement).getAttribute('data-page-break') != null
        if (isBreak) {
          pages.push(buf)
          buf = ''
          used = 0
        ...
        const hasImage =
          (node as HTMLElement).tagName.toUpperCase() === 'IMG' ||
      ...
      try {
        ...
        pages.forEach((htmlStr, idx) => {
          ...
          console.log(
            `[CardPreview] page ${idx + 1}/${pages.length} height:`,
            h,
            'innerH(target):',
            innerH
          )
  ...
  const onExport = useCallback(async () => {
    const extraCss =
      `.mdx-content{font-size:${16 * fontScale}px}` +
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div className="ui-toolbar">
        ...
        <div className="ui-toolbar-group" style={{ display: 'none' }}>
          ...
          <label className="ui-field">
            ...
            />
            <span style={{ fontSize: 12 }}>{Math.round(previewScale * 100)}%</span>
          </label>
          <label>
            长宽比：
            <span className="ui-select-wrap">
              <select
                className="ui-select"
                value={ratioPreset}
                onChange={(e) => setRatioPreset(e.target.value as any)}
                style={{ marginLeft: 6 }}
              >
          ...
          {ratioPreset === 'custom' && (
            <label>
              ...
              <input
                ...
                step={0.01}
                value={customRatio}
                onChange={(e) => setCustomRatio(Math.max(0.1, Number(e.target.value) || 4 / 3))}
                style={{ width: 90, marginLeft: 6 }}
              />
            </label>
          )}
          ...
          <label>
            ...
            <input
              ...
              value={canvasPadding}
              onChange={(e) =>
                setCanvasPadding(Math.max(0, Math.min(48, Number(e.target.value) || 0)))
              }
          ...
          {bgMode === 'solid' && (
            <label>
              ...
              <input
                ...
                style={{ width: 60, height: 32, padding: 0, marginLeft: 6 }}
              />
            </label>
          )}
          {bgMode === 'gradient' && (
            <label>
              ...
              <span className="ui-select-wrap">
                <select
                  className="ui-select"
                  value={bgGradientKey}
                  onChange={(e) => setBgGradientKey(e.target.value)}
                  style={{ marginLeft: 6 }}
                >
            ...
            <>
              ...
              <label className="ui-field">
                ...
                <input
                  ...
                  step={0.01}
                  value={bgMask}
                  onChange={(e) => setBgMask(Number(e.target.value))}
                />
                <span style={{ fontSize: 12 }}>{Math.round(bgMask * 100)}%</span>
              </label>
            </>
          )}
        </div>
          ...
          <label>
            ...
            <span className="ui-select-wrap">
              ...
              >
                ...
                <option value={'"PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", Arial'}>
                  中文优先
                </option>
        ...
        <div className="ui-toolbar-group">
          ...
          <button className="ui-btn ui-btn--blue" onClick={onExportAll}>
            导出全部
          </button>
          <button className="ui-btn ui-btn--outline" onClick={onClientExport}>
            前端导出
          </button>
          <button
            className="ui-btn ui-btn--teal"
            onClick={() => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 0, flex: 1 }}>
                  ...
                  {ratioPreset === 'custom' && (
                    <label>
                      ...
                      <input
                        ...
                        value={customRatio}
                        onChange={(e) =>
                          setCustomRatio(Math.max(0.1, Number(e.target.value) || 4 / 3))
                        }
                        style={{ width: 120, marginLeft: 6 }}
                      />
                    </label>
                  ...
                  <label>
                    ...
                    <span className="ui-select-wrap">
                      ...
                      >
                        ...
                        <option
                          value={'"PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", Arial'}
                        >
                          中文优先
                        </option>
                        <option value={'Georgia, "Times New Roman", serif'}>衬线 Serif</option>
          ...
          <div style={{ display: 'none' }}>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setPreviewContainerWidth(Math.max(200, Number(e.target.value) || 500))
                }
                style={{ width: 90, marginLeft: 6 }}
              />
            </label>
            ...
            <label>
              ...
              <input
                type="number"
                value={pageGap}
                onChange={(e) => setPageGap(Math.max(0, Number(e.target.value) || 0))}
            ...
            <label>
              ...
              <input
                type="number"
                value={borderRadius}
                onChange={(e) => setBorderRadius(Math.max(0, Number(e.target.value) || 0))}
                style={{ width: 80, marginLeft: 6 }}
              />
            </label>
            ...
            <button
              onClick={onExportAll}
              style={{ padding: '6px 10px', borderRadius: 8, background: '#0ea5e9', color: '#fff' }}
            >
              导出全部
            </button>
            <button
            ...
            <button
              ...
              style={{ padding: '6px 10px', borderRadius: 8, background: '#10b981', color: '#fff' }}
            >
              新窗口预览
            </button>
          </div>
        ...
        >
          {(() => {
            const totalScale = (previewContainerWidth / pageWidth) * previewScale
            const scaledWidth = Math.round(pageWidth * totalScale)
            return (
              <div
                style={{
              ...
              >
                <div style={{ width: scaledWidth }}>
                  ...
                  >
                    {pagesHtml.map((htmlStr, idx) => (
                      <div
                        key={idx}
                        style={{ margin: `0 auto ${pageGap}px auto`, position: 'relative' }}
                      >
                        <CardXhs
                          width={pageWidth}
                          padding={canvasPadding}
                          borderRadius={borderRadius}
                          backgroundImageUrl={bgMode === 'image' ? bgUrl : ''}
                          backgroundImageFit={bgFit}
                          backgroundMaskOpacity={bgMask}
                          style={cardBgStyle}
                        >
                          <div
                            className="mdx-content"
                            style={{ fontSize: `${16 * fontScale}px`, fontFamily }}
                ...
                <div
                  ref={measureRef}
                  style={{ position: 'absolute', left: -99999, top: -99999, visibility: 'hidden' }}
                />
              </div>
            )
          })()}
